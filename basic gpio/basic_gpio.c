...
void GPIO_init(const GPIO_Configuration * config){
    SET_BIT(SYSCTL_RCGCGPIO_R, config->port); /* enable clock for the port */
    if (config->port == PORTA){
        /* Configurate direction & data */
        if (config->dir == Output){
            SET_BIT(GPIO_PORTA_DIR_R, config->pin);

            if (config->data){ SET_BIT(GPIO_PORTA_DATA_R, config->pin);}
            else{ CLEAR_BIT(GPIO_PORTA_DATA_R, config->pin);}
        ...
        if (config->internalR != Disable_internal_resistor){
            ...
            GPIO_PORTA_LOCK_R = 0x4C4F434B; /* unlock commit reg. */
            SET_BIT(GPIO_PORTA_CR_R,config->pin) ; /* enable commit reg. to control PUR/PDR reg. */
        ...
        if (config->dir == Output){
            ...
            if (config->data){ SET_BIT(GPIO_PORTB_DATA_R, config->pin);}
            else{ CLEAR_BIT(GPIO_PORTB_DATA_R, config->pin);}
        ...
        if (config->internalR != Disable_internal_resistor){
            ...
            GPIO_PORTB_LOCK_R = 0x4C4F434B; /* unlock commit reg. */
            SET_BIT(GPIO_PORTB_CR_R,config->pin) ; /* enable commit reg. to control PUR/PDR reg. */
        ...
        if (config->dir == Output){
            ...
            if (config->data){ SET_BIT(GPIO_PORTC_DATA_R, config->pin);}
            else{ CLEAR_BIT(GPIO_PORTC_DATA_R, config->pin);}
        ...
        if (config->internalR != Disable_internal_resistor){
            ...
            GPIO_PORTC_LOCK_R = 0x4C4F434B; /* unlock commit reg. */
            SET_BIT(GPIO_PORTC_CR_R,config->pin) ; /* enable commit reg. to control PUR/PDR reg. */
        ...
        if (config->dir == Output){
            ...
            if (config->data){ SET_BIT(GPIO_PORTD_DATA_R, config->pin);}
            else{ CLEAR_BIT(GPIO_PORTD_DATA_R, config->pin);}
        ...
        if (config->internalR != Disable_internal_resistor){
            ...
            GPIO_PORTD_LOCK_R = 0x4C4F434B; /* unlock commit reg. */
            SET_BIT(GPIO_PORTD_CR_R,config->pin) ; /* enable commit reg. to control PUR/PDR reg. */
        ...
        if (config->dir == Output){
            ...
            if (config->data){ SET_BIT(GPIO_PORTE_DATA_R, config->pin);}
            else{ CLEAR_BIT(GPIO_PORTE_DATA_R, config->pin);}
        ...
        if (config->internalR != Disable_internal_resistor){
            ...
            GPIO_PORTE_LOCK_R = 0x4C4F434B; /* unlock commit reg. */
            SET_BIT(GPIO_PORTE_CR_R,config->pin) ; /* enable commit reg. to control PUR/PDR reg. */
        ...
        if (config->dir == Output){
            ...
            if (config->data == LOGIC_HIGH){ SET_BIT(GPIO_PORTF_DATA_R, config->pin);}
            else{ CLEAR_BIT(GPIO_PORTF_DATA_R, config->pin);}
        ...
        if (config->internalR != Disable_internal_resistor){
            ...
            SET_BIT(GPIO_PORTF_CR_R,config->pin) ; /* enable commit reg. to control PUR/PDR reg. */
